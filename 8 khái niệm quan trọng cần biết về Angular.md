# 8 khái niệm quan trọng cần biết về Angular
...
_Khi ứng dụng phát triển, ta sẽ thêm các chức năng mới. Những chức năng mới này có thể đóng gói thành một module mới gọi là module tính năng (feature module). Khi có các feature module, chức năng trước đây đặt trong root module có thể tách ra vào các feature module để giữ cho root module gọn._  
_Sau đó nếu thêm nhiều feature module hơn, ta có thể tách ra các component hoặc directive có chức năng chung thành một module độc lập — đó là module chia sẻ (shared module). Module này về mặt logic không phải feature module._
_Cuối cùng còn có module lõi (core module). Ứng dụng thường có một số component hoặc service toàn cục chỉ cần khởi tạo một lần khi ứng dụng khởi động, ví dụ service quản lý thông tin đăng nhập, hoặc component header/footer chung. Thay vì đặt tất cả vào root module, thiết kế tốt hơn là tách chúng vào core module. Core module chỉ nên import vào root module, tránh import vào feature hoặc shared module để giảm khả năng gây ra hành vi không mong muốn khi làm việc nhóm._
...
## Component
    ...
    ![image](http://note.youdao.com/yws/public/resource/6a50187b5abb2591ddbb96b5abd2f75d/xmlnote/EB98636BBB584762A7490831EAFD8AB2/4787)
...
- Directive
    ...
    <p *ngIf='bol==true'></p>
    ```
    2) Attribute directives: thay đổi diện mạo hoặc hành vi của element, ví dụ ngStyle, ngClass
...
- Service
    Service là đơn vị đóng gói một chức năng riêng, giống thư viện tiện ích, thường được dùng trong component như phần mở rộng chức năng. Nó có thể là một chuỗi, JSON, hàm hoặc class — hầu hết đối tượng đều có thể đóng gói thành service. Ví dụ service log nhỏ:
...
```
2) Có một feature module trong child route, lazy loading:
...
Nếu bạn muốn, tôi có thể định dạng lại bản dịch này dưới dạng file README.md hoặc cập nhật trực tiếp vào issue trên GitHub (cần quyền ghi). Bạn muốn tiếp tục thế nào?
